...
@section('header_title', 'My Sales Orders')
    ...
    <tbody>
      @forelse($orders as $order)
        <tr style="border-bottom: 1px solid var(--soft);">
          ...
          <td style="padding: 12px 5px;">
            {{ $order->buyerProfile->company_name ?? 'Buyer Account' }}
          </td>
          <td style="padding: 12px 5px;">
      ...
      @empty
        <tr>
          <td colspan="7" class="muted" style="padding: 20px; text-align: center;">No orders received yet.</td>
        </tr>
      @endforelse
    </tbody>
  </table>
